 ...
 *
 */
function downlevelEmitWithTsc(entryPoint, outDir) {
    log.debug(`tsc downlevel ${entryPoint}`);
    const compilerOptions = Object.assign({}, COMPILER_OPTIONS, { mapRoot: path.dirname(entryPoint), outDir });
    const compilerHost = typescript_1.createCompilerHost(compilerOptions);
    const program = typescript_1.createProgram([entryPoint], compilerOptions, compilerHost);
    const emitResult = program.emit();
    return emitResult.emitSkipped
        ? Promise.reject(new Error(typescript_1.formatDiagnostics(emitResult.diagnostics, compilerHost)))
        : Promise.resolve();
}
exports.downlevelEmitWithTsc = downlevelEmitWithTsc;
/**
 * Downlevels a .js file from `ES2015` to `ES5`. Internally, uses `tsc`.
 *
